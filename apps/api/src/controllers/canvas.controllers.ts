...
import { createCanvasSchema, canvasParamsSchema } from "@matrimo/schemas";

import {
  createCanvas,
  findCanvasById,
  updateCanvas,
} from "../data/canvas.data.js";
...
const get = apiHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(true, "Canvas found", { canvas }));
  } catch (error) {
...
const update = apiHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json(ApiResponse.success("Canvas updated", { canvas }));
  } catch (error) {
