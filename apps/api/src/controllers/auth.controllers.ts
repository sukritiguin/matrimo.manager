...
export const register = apiHandler(async (req: Request, res: Response) => {
  try {
    ...
    logger.debug("ERROR IN REGISTER", error);
...
export const login = apiHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof ApiError) throw error;
    logger.debug("ERROR IN LOGIN", error);
...
export const logout = apiHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof ApiError) throw error;
    logger.debug("ERROR IN LOGOUT", error);
    throw new ApiError(500, "Sign out failed");
  }
});
...
export const refreshToken = apiHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      currentSession.expires < new Date(Date.now())
    ) {
    ...
    if (error instanceof ApiError) throw error;
    logger.debug("ERROR IN REFRESH TOKEN", error);
...
export const googleCallback = apiHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof ApiError) throw error;
      logger.debug("ERROR IN GOOGLE CALLBACK", error);
      throw new ApiError(500, "Sign in failed");
    }
  }
);
