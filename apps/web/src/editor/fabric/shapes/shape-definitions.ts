...
export interface IShapeDefinition {
  ...
  label: string;
  defaultProps: any;
  thumbnail: (fabricInstance: typeof fabric, canvas: fabric.Canvas | fabric.StaticCanvas) => void;
}

export const shapeDefinitions: Record<string, IShapeDefinition> = {
  rectangle: {
    type: "rect",
    label: "Rectangle",
    defaultProps: {
  ...
  arrow: {
    ...
    thumbnail: (fabric, canvas) => {
      const { Path } = fabric;
      const arrowPath = "M 10,45 L 65,45 L 65,30 L 90,50 L 65,70 L 65,55 L 10,55 z";
